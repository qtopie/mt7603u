...
VOID CmdSetTxPowerCtrl(RTMP_ADAPTER *pAd, UINT8 central_chl)
{
	...
	else
	{
		...
		NdisCopyMemory(&CmdTxPwrCtrl.aucCHPowerDelta[0], &(pAd->EEPROMImage[TX0_G_BAND_OFFSET_LOW]), sizeof(CmdTxPwrCtrl.aucCHPowerDelta));
		NdisCopyMemory(&CmdTxPwrCtrl.aucCHPowerDelta[1], &(pAd->EEPROMImage[TX0_G_BAND_CHL_PWR_DELTA_MID]), sizeof(CmdTxPwrCtrl.aucCHPowerDelta));
		NdisCopyMemory(&CmdTxPwrCtrl.aucCHPowerDelta[2], &(pAd->EEPROMImage[TX0_G_BAND_OFFSET_HIGH]), sizeof(CmdTxPwrCtrl.aucCHPowerDelta));
		NdisCopyMemory(&CmdTxPwrCtrl.aucCHPowerDelta[3], &(pAd->EEPROMImage[TX1_G_BAND_CHL_PWR_DELATE_LOW]), sizeof(CmdTxPwrCtrl.aucCHPowerDelta));

		NdisCopyMemory(&CmdTxPwrCtrl.aucCHPowerDelta[4], &(pAd->EEPROMImage[TX1_G_BAND_CHL_PWR_DELTA_MID]), sizeof(CmdTxPwrCtrl.aucCHPowerDelta));

		NdisCopyMemory(&CmdTxPwrCtrl.aucCHPowerDelta[5], &(pAd->EEPROMImage[TX1_G_BAND_CHL_PWR_DELTA_HIGH]), sizeof(CmdTxPwrCtrl.aucCHPowerDelta));
...
INT32 CmdExtPmStateCtrl(RTMP_ADAPTER *pAd, UINT8 ucWlanIdx, UINT8 ucPmNumber, UINT8 ucPmState)
{
	...
	NdisMoveMemory(CmdPmStateCtrl.aucBssid, pAd->CommonCfg.Bssid, MAC_ADDR_LEN);
	DBGPRINT(RT_DEBUG_OFF, ("%s:(%x, %x, %x, %x, %x, %x), BeaconPeriod(%d), DtimPeriod(%d), AID(%d)\n",
								...
								//pAd->MlmeAux.DtimPeriod, //Pat: diff from 7636
	...
	{
		//CmdPmStateCtrl.ucDtimPeriod = cpu2le32(pAd->MlmeAux.DtimPeriod); //Pat: diff from 7636
		CmdPmStateCtrl.ucDtimPeriod = cpu2le32(pAd->StaCfg.DtimPeriod);
		...
		u4RfCr &= ~DROP_NOT_MY_BSSID; //Pat: diff from 7636
		u4RfCr &= ~DROP_DIFF_BSSID_BCN; //Pat: diff from 7636
		CmdPmStateCtrl.u4RxFilter = cpu2le32(u4RfCr);
		DBGPRINT(RT_DEBUG_OFF, ("%s: u4RxFilter 0x%x\n", __FUNCTION__, CmdPmStateCtrl.u4RxFilter));

		CmdPmStateCtrl.u4Feature = cpu2le32((PM_CMD_FEATURE_PSPOLL_OFFLOAD | PM_CMD_FEATURE_PS_TX_REDIRECT | PM_CMD_FEATURE_SMART_BCN_SP));
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	// Set magic packet enable
	NdisZeroMemory(&CmdMagicPacket, sizeof(CMD_PACKET_FILTER_MAGIC_PACKET_T));

	msg = AndesAllocCmdMsg(pAd, sizeof(CMD_PACKET_FILTER_MAGIC_PACKET_T));
	...
	CmdMagicPacket.PFType = cpu2le32(_ENUM_TYPE_MAGIC);
	CmdMagicPacket.BssidEnable = cpu2le32(0xf);

	AndesAppendCmdMsg(msg, (char *)&CmdMagicPacket, sizeof(CMD_PACKET_FILTER_MAGIC_PACKET_T));
	...
	CmdPFGlobal.FunctionSelect = cpu2le32(_ENUM_GLOBAL_EAPOL);
	CmdPFGlobal.Enable = cpu2le32(1);
	...
	CmdPFGlobal.PFType = cpu2le32(_ENUM_TYPE_GLOBAL_EN);
	CmdPFGlobal.FunctionSelect = cpu2le32(_ENUM_GLOBAL_MODE);
	CmdPFGlobal.Enable = cpu2le32(PF_MODE_WHITE_LIST); //white
